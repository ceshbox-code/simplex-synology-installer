...
command -v openssl &>/dev/null || error "openssl не найден."
command -v curl    &>/dev/null || error "curl не найден."
command -v docker  &>/dev/null || error "Docker не установлен. Установите через Package Center."
command -v ip      &>/dev/null || error "ip не найден."

COMPOSE_CMD="docker compose"
if ! docker compose version &>/dev/null; then
    COMPOSE_CMD="docker-compose"
    command -v docker-compose &>/dev/null || error "Docker Compose не найден."
fi
...
echo ""
echo "Доступные варианты установки:"
for opt in "${OPTIONS[@]}"; do echo "  $opt"; done
echo ""
while true; do
    read -p "Выберите номер варианта [1]: " CHOICE < /dev/tty
    CHOICE=${CHOICE:-1}
...
read -p "Введите внешний IP-адрес [$EXTERNAL_IP]: " INPUT_EXT_IP < /dev/tty
EXTERNAL_IP=${INPUT_EXT_IP:-$EXTERNAL_IP}
[ -z "$EXTERNAL_IP" ] && error "Не удалось определить внешний IP."
[ -z "$INTERNAL_IP" ] && error "Не удалось определить внутренний IP NAS."

while true; do
    read -p "Введите доменное имя (например, ваш-домен.ru): " MAIN_DOMAIN < /dev/tty
    if [ -z "$MAIN_DOMAIN" ]; then warn "Домен не может быть пустым."; continue; fi
    DOTS="${MAIN_DOMAIN//[^.]}"
    if [ -z "$DOTS" ]; then warn "Домен должен содержать точку."; continue; fi
    break
done

read -p "Email администратора [admin@$MAIN_DOMAIN]: " ADMIN_EMAIL < /dev/tty
...
# 3. ДИРЕКТОРИИ
# ==============================================================================
info "Подготовка структуры директорий..."
...
if [ ! -s "$BASE_DIR/smp/certificates/$SMP_DOMAIN.crt" ] || [ ! -s "$BASE_DIR/smp/certificates/$SMP_DOMAIN.key" ]; then
    ...
    success "Новый self-signed сертификат SMP создан."
else
    warn "Существующий сертификат SMP сохранён."
fi
...
success "TLS сертификаты SMP готовы."
...
# ==============================================================================
info "Проверка инициализации XFTP сервера..."
...
if [ -s "$XFTP_FP_FILE" ] && [ "$(head -1 "$XFTP_FP_FILE")" != "PENDING" ]; then
    ...
    info "Запуск инициализации XFTP..."
    ...
    if ! docker run -d --name "$XFTP_INIT_NAME" \
        ...
        error "Не удалось запустить XFTP init."
    fi
    ...
    if [ ! -s "$XFTP_FP_FILE" ] || [ "$(head -1 "$XFTP_FP_FILE")" = "PENDING" ]; then
        ...
        docker rm -f "$XFTP_INIT_NAME" >/dev/null 2>&1 || true
        error "Не удалось получить XFTP fingerprint."
    fi

    XFTP_FP_INIT=$(head -1 "$XFTP_FP_FILE")
    docker rm -f "$XFTP_INIT_NAME" >/dev/null 2>&1 || true
    success "XFTP identity создана: $XFTP_FP_INIT"
    success "XFTP инициализирован."
fi
...
# ==============================================================================
cd "$BASE_DIR"
info "Загрузка зафиксированных Docker-образов..."
if ! $COMPOSE_CMD pull; then
    error "Не удалось загрузить Docker-образы."
fi

info "Запуск сервисов..."
if ! $COMPOSE_CMD up -d; then
    error "Docker Compose не смог запустить сервисы."
fi
...
# ==============================================================================
info "Ожидание генерации fingerprint (до 120 сек)..."
sleep 10
WAIT=0; SMP_FP=""
...
if [ -z "$SMP_FP" ] || [ "$SMP_FP" = "PENDING" ]; then
    warn "SMP fingerprint ещё не сгенерирован."
...
if [ -z "$XFTP_FP" ] || [ "$XFTP_FP" = "PENDING" ]; then
    warn "XFTP fingerprint ещё не сгенерирован."
...
success "Fingerprint получены."

# ==============================================================================
# 10. BACKUP
# ==============================================================================
info "Создание скрипта резервного копирования..."
...
success "Backup-скрипт создан."

# ==============================================================================
# 11. STATUS-UPDATE.SH + CRON
# ==============================================================================
info "Создание скрипта обновления статуса..."
...
if ! grep -q "status-update.sh" /etc/crontab 2>/dev/null; then
    ...
    success "Cron-задача статуса добавлена (каждую минуту)."
else
    warn "Cron-задача status-update.sh уже существует."
fi
...
if ! grep -q "simplex-backup.sh" /etc/crontab 2>/dev/null; then
    ...
    synoservicectl --restart crond 2>/dev/null || true
    success "Cron-задача backup добавлена (ежедневно в 03:00)."
else
    warn "Cron-задача simplex-backup.sh уже существует."
fi
success "status-update.sh создан."

# ==============================================================================
# 12. WEB-ФАЙЛЫ
# ==============================================================================
info "Создание веб-страниц..."
QRCODE_JS="$WEB_DIR/qrcode.min.js"
if [ ! -s "$QRCODE_JS" ]; then
    info "Загрузка QRCode.js..."
    if curl -fsSL --max-time 20 \
        "https://install.smp.klenovoe.ru/qrcode.min.js" \
        -o "$QRCODE_JS" 2>/dev/null; then
        chmod 644 "$QRCODE_JS"
        success "QRCode.js загружен."
    else
        error "Не удалось загрузить QRCode.js. Проверьте доступность install.smp.klenovoe.ru"
    fi
fi
chmod 644 "$QRCODE_JS"
...
if [ -f "$WEB_DIR/index.html" ]; then
    ...
    chmod 644 "$WEB_DIR/index.html"
    success "Версия существующего index.html обновлена до v9.4."
fi
...
success "Веб-файлы созданы в $WEB_DIR"
...
Логин: admin
Пароль: ${WEB_PASS:-см. существующий .htpasswd}

📁 ФАЙЛЫ:
Конфигурация: $BASE_DIR/.env
Веб-страницы: $WEB_DIR/
Backup: $BACKUP_SCRIPT
Статус: $BASE_DIR/status-update.sh

⚠️ ДАЛЬНЕЙШИЕ ДЕЙСТВИЯ:
1. Настройте Web Station: корень → $WEB_DIR
2. Проброс портов: SMP 5223, 5224, XFTP 7788, TURN 3478, 5349, 49152-65535
3. DNS A-записи: smp, files, turn → $EXTERNAL_IP
4. Откройте порты в Брандмауэре DSM
================================================================
EOF
